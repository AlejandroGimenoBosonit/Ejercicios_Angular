...
export class FormComponent implements OnInit {
  ...
  myForm: FormGroup = this.fb.group(
    {
      ...
      email: [
        ...
        [
          Validators.required,
          Validators.pattern(this.validations.emailInputPattern),
        ],
        [this.usersService],
      ],
  ...
  submitForm() {
    ...
    if (this.editMode) {
      // edit
      this.usersService.updateUser(this.myForm.value).subscribe({
        next: () => {
      ...
      // add mode
      this.usersService.postUser(this.myForm.value).subscribe({
        next: () => {
  ...
  operateAction(event: formControls | number) {
    if (typeof event === 'number') {
      // delete mode
      this.usersService.deleteUserById(event).subscribe({
        next: () => {
          console.log('User updated successdully');
          this.getUsers();
          this.myForm.reset();
